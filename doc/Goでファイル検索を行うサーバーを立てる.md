...
# まとめ・補足
* モデルはGoogle検索です。シンプルな検索フォームだけのデザインと、他人の検索結果を表示できるようにする機能をまねしました。
...
* せっかくのRESTFUL APIサーバー、JSONをパースできればなんでもいいので、クライアントサイドは別途デスクトップアプリを用意したほうがいいかも。
* 本番環境での検索時間は4-6秒(ファイル数600万件, ntfs形式,おそらくHDD)ですが、体感もっと早くしたいです。locateコマンドは標準出力に逐次結果を表示しているので、Ajaxで非同期に結果を表示できないのか？できるでしょ？
* キャッシュを使うとDBを検索せず、メモリにアクセスするだけなので、マイクロ秒オーダーで検索結果を返せます。そこでユーザーになるべくキャッシュを使ってもらうようにします。(リソースも使わないのでエコ)
* 
